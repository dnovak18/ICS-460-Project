...
class SenderThread extends Thread {
	...
	SenderThread(DatagramSocket socket, InetAddress address, int port,
			SenderPacketHandler handler, Sender sender) {
		...
		this.timeoutInterval = sender.getTimeoutInterval();
	...
	public void sendPacket(int seqno) {
		...
		if (!handler.failureCheck(seqno)) {
			...
			try {
				sender.setOutputMessage("Client sending packet no " + seqno);
				socket.send(output);
				if (seqno < 0) {
					sender.setOutputMessage("Client: End of file reached");
				}
			}
	...
	class SendTask extends TimerTask {
		...
		public synchronized void run() {
			sender.setOutputMessage("Packet " + seqno + " timed out; resending");
			sendPacket(seqno);
		}
	}
